...
// Fonction pour vérifier si un livreur existe déjà dans la base de données
Livreur.exists = async (email) => {
  try {
    const pool = await sql.connect(config);
    const result = await pool.request().input('email', sql.NVarChar, email).query(`SELECT * FROM ${Livreur.tableName} WHERE email = @email`);
...
Livreur.checkPassword = async (email, password) => {
  try {
    const pool = await sql.connect(config);
    const result = await pool.request().input('email', sql.NVarChar, email).query(`SELECT hashedPassword FROM ${Livreur.tableName} WHERE email = @email`);
...
// Fonction pour récupérer un livreur par son ID depuis la base de données
Livreur.getById = async (id) => {
  try {
    const pool = await sql.connect(config);
    const result = await pool.request().input('id', sql.UniqueIdentifier, id).query(`SELECT * FROM ${Livreur.tableName} WHERE LivreurID = @id`);
    return result.recordset[0];
  } catch (err) {
    throw new Error(err.message);
  }
